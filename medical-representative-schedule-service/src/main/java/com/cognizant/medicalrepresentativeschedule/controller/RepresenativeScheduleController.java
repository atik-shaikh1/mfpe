...
@Slf4j
@RestController
public class RepresenativeScheduleController {
	...
	public ResponseEntity<List<RepSchedule>> getRepSchedule(@RequestHeader(name = "Authorization") String token, @PathVariable("scheduleStartDate") String scheduleStartDate)
	...
	public ResponseEntity<String[]> getMedicinesByTreatingAilment(@RequestHeader(name = "Authorization") String token) {
		...
		ResponseEntity<String[]> responseEntity = ResponseEntity.of(Optional.of(medicineStockClient.getMedicinesByTreatingAilment(token, "medicine")));
	...
	public List<MedicalRepresentative> getMedicalRepresentatives(@RequestHeader(name = "Authorization") String token) throws TokenValidationFailedException {
		log.info("Start");
